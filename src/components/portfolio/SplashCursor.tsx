...
export const SplashCursor = ({
  SIM_RESOLUTION = 128,
  DYE_RESOLUTION = 1024, // Optimized default for better baseline GPU performance
  CAPTURE_RESOLUTION = 512,
  DENSITY_DISSIPATION = 3.5,
  VELOCITY_DISSIPATION = 2,
  PRESSURE = 0.8,
  PRESSURE_ITERATIONS = 20,
  CURL = 30, // Higher turbulency for cyberpunk aesthetic
  SPLAT_RADIUS = 0.2,
  SPLAT_FORCE = 6000,
  SHADING = true,
  ...
  useEffect(() => {
    ...
    class PointerItem implements Pointer {
      id = -1;
      texcoordX = 0;
      texcoordY = 0;
      prevTexcoordX = 0;
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement): { gl: WebGL2RenderingContext | WebGLRenderingContext; ext: WebGLExtensions } | null {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
      ...
      let glCtx: WebGL2RenderingContext | WebGLRenderingContext | null = canvasEl.getContext('webgl2', params) as WebGL2RenderingContext | null;
      const isWebGL2 = !!glCtx;
      if (!glCtx) {
        glCtx = (canvasEl.getContext('webgl', params) as WebGLRenderingContext | null) || (canvasEl.getContext('experimental-webgl', params) as WebGLRenderingContext | null);
      }
      ...
      const halfFloatTexType = isWebGL2 ? (glCtx as WebGL2RenderingContext).HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES ? halfFloat.HALF_FLOAT_OES : glCtx.UNSIGNED_BYTE;
    ...
    function getSupportedFormat(glCtx: WebGLRenderingContext | WebGL2RenderingContext, internalFormat: number, format: number, type: number): WebGLFormat | null {
      if (!supportRenderTextureFormat(glCtx, internalFormat, format, type)) {
        // Safe check for WebGL2 specific constants
        const RG16F = (glCtx as WebGL2RenderingContext).RG16F || 0x822F;
        const R16F = (glCtx as WebGL2RenderingContext).R16F || 0x822D;
        const RGBA16F = (glCtx as WebGL2RenderingContext).RGBA16F || 0x881A;
        const RG = (glCtx as WebGL2RenderingContext).RG || 0x8227;

        switch (internalFormat) {
          case R16F:
            return getSupportedFormat(glCtx, RG16F, RG, type);
          case RG16F:
    ...
    function supportRenderTextureFormat(glCtx: WebGLRenderingContext | WebGL2RenderingContext, internalFormat: number, format: number, type: number): boolean {
    ...
    function compileShader(type: number, source: string, keywords?: string[]): WebGLShader | null {
      const modifiedSource = addKeywords(source, keywords);
  ...
  return (
    <div
      style={{
        ...
        top: 0,
        left: 0,
        zIndex: 10, // Sits above BackgroundDecorations (which is zIndex: 0), but below interactive UI elements (usually zIndex >= 20/30/40/50)
        pointerEvents: 'none', // Critical: ensures links and buttons underneath remain fully clickable
        width: '100%',
        height: '100%'
      }}
    >
